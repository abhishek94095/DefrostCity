Shader "WBGB/Lit/Diff Norm MSA"
{
    Properties
    {
        ...
        [NoScaleOffset]
        _MetalSmoothAO("Metallic (R), Smoothness (G), Ambient Occlusion (B)", 2D) = "bump"{}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline void InitializeSurfaceData(float2 uv, out SurfaceData outSurfaceData)
            {
                ...
                half4 msa = half4(SAMPLE_TEXTURE2D(_MetalSmoothAO, sampler_MetalSmoothAO, uv / _Tiling));
                outSurfaceData.albedo = albedoAlpha.rgb;
                outSurfaceData.metallic = msa.r;
                outSurfaceData.specular = half3(0.0, 0.0, 0.0);

                outSurfaceData.smoothness = msa.g;
                outSurfaceData.normalTS = UnpackNormal(SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, uv / _Tiling));
